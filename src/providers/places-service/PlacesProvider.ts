...
export class PlacesProvider {
  ...
  sortingAndFiltering(places: Place[], searchObject: {
    ...
    return this.getAllPlaces().map(places => {
      ...
      places = places.filter(places => {
        return places.averagePrice >= searchObject.range.lower && places.averagePrice <= searchObject.range.upper;
      })
      ...
      for (const filterFeaturesKey in filterFeatures) {
        if (filterFeatures[filterFeaturesKey]) filterObject[filterFeaturesKey] = filterFeatures[filterFeaturesKey];
      }
